...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let { data } = $$props;
  const filteredMatches = $matches.filter((match) => match.stage_name === "Round of 16");
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  $$unsubscribe_matches();
  return `<section class="${"grid md:grid-cols-2 lg:grid-cols-3 place-items-center"}">${each(filteredMatches, (match) => {
    return `${validate_component(Match, "Match").$$render($$result, { match, link: data.link.data }, {}, {})}`;
  })}</section>`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-3bb15e1a.js.map
