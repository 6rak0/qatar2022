...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let { data } = $$props;
  const third = $matches.filter((match) => match.stage_name === "Play-off for third place");
  const final = $matches.filter((match) => match.stage_name === "Final");
  const finals = [...third, ...final];
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  $$unsubscribe_matches();
  return `<section class="${"grid md:grid-cols-2 place-items-center"}">${each(finals, (match) => {
    return `${validate_component(Match, "Match").$$render($$result, { match, link: data.link.data }, {}, {})}`;
  })}</section>`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-d40f4dc0.js.map
